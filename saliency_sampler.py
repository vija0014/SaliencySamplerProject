...
class Saliency_Sampler(nn.Module):
    def __init__(self,task_network,saliency_network,task_input_size,saliency_input_size,last_conv_size=256):
        ...
        self.conv_last = nn.Conv2d(last_conv_size,1,kernel_size=1,padding=0,stride=1)
    ...
    def create_grid(self, x):
        ...
        grid = nn.functional.interpolate(grid,size=(self.input_size_net,self.input_size_net),mode='bilinear',align_corners=False)
    ...
    def forward(self, x,p):
        ...
        xs = nn.functional.interpolate(xs,size=(self.grid_size,self.grid_size),mode='bilinear',align_corners=False)
        ...
        if random.random()>p:
            ...
            x_sampled = nn.functional.interpolate(x_sampled,size=(self.input_size_net,self.input_size_net),mode='bilinear',align_corners=False)
